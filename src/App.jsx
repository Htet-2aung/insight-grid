...
import { Loader, BarChart2, Clock, Users, Search, User, LogOut, PlusCircle, UploadCloud, Trash2, X, ArrowLeft, FileText, TestTube2, MessageSquare, Send, Globe, Edit, Code, Save, XCircle, Settings } from 'lucide-react';
import './App.css'; 

// Assuming NotebookRender.js exists. If not, these are placeholders.
...
const analyzeNotebookWithLLM = async (rawNotebookContent) => {
    // This is a placeholder for your actual notebook analysis logic.
    // For now, it returns a mock object.
    console.log("Analyzing notebook (mock)...");
    return {
        title: "Mock Analyzed Project",
        description: "This is a mock description from a notebook.",
...
const Dashboard = ({ user, onSelectProject, onNavigate }) => {
    ...
    const handleFileUpload = async (event) => {
        ...
        reader.onload = async (e) => {
            ...
            try { JSON.parse(rawContent); } catch (parseError) { alert("The uploaded file is not a valid JSON .ipynb file."); setIsAnalyzing(false); return; }
        ...
        <>
            ...
            <div className="page-container">
                <div className="dashboard-header">
                    <div className="welcome-message"><h2>Welcome back, {user.email.split('@')[0]}!</h2><p>Here's an overview of your projects.</p></div>
                ...
                <div className="dashboard-actions">
                    ...
                    <button onClick={() => fileInputRef.current.click()} className="action-button" disabled={isAnalyzing}>{isAnalyzing ? <><Loader size={16} className="animate-spin" /> Analyzing...</> : <><UploadCloud size={16} /> Analyze Notebook</>}</button>
                </div>
                <main className="projects-grid">
                    {loading ? <p>Loading projects...</p> : projects.length > 0 ? projects.map(project => (<ProjectCard key={project.id} project={project} onClick={() => onSelectProject(project.id)} onDelete={deleteProject} />)) : <p>No projects yet. Add one manually or analyze a notebook!</p>}
                </main>
...
const ProjectDetailPage = ({ projectId, onNavigate, user }) => {
    ...
    useEffect(() => {
        const fetchProjectData = async () => {
            ...
            if (projectError) console.error("Error fetching project:", projectError);
                ...
                setEditableProject({ title: projectData.title, description: projectData.description, methodology: projectData.methodology, tags: (projectData.tags || []).join(', ') });
            }
            setLoading(false);
        };
        fetchProjectData();
    ...
    const handleUpdateProject = async () => {
        ...
        else { setProject(data); setAuthor(data.profiles); setIsEditing(false); }
        setIsSaving(false);
    };
        ...
        <>
            ...
            <div className="page-container">
                ...
                <div className="project-detail-header">
                    {isEditing ? <input name="title" value={editableProject.title} onChange={handleInputChange} className="title-input" /> : <h1>{project.title}</h1>}
                ...
                {isEditing ? (
                    <div className="edit-form">
                        <label>Description</label><textarea name="description" value={editableProject.description} onChange={handleInputChange} rows="6" />
                        <label>Methodology</label><textarea name="methodology" value={editableProject.methodology} onChange={handleInputChange} rows="4" />
                        <label>Tags (comma-separated)</label><input name="tags" value={editableProject.tags} onChange={handleInputChange} />
                        <div className="edit-actions">
                            <button onClick={handleUpdateProject} className="button-primary" disabled={isSaving}>{isSaving ? <><Loader size={16} className="animate-spin"/> Saving...</> : <><Save size={16}/> Save Changes</>}</button>
            ...
            <div className="page-container">
                ...
                <div className="edit-profile-form">
                    ...
                    <form onSubmit={handleUpdateProfile}>
                        <div className="form-group"><label htmlFor="username">Username</label><input id="username" name="username" value={profile.username} onChange={handleInputChange} placeholder="e.g., dataviz_guru" /></div>
                        <div className="form-group"><label htmlFor="title">Title / Headline</label><input id="title" name="title" value={profile.title} onChange={handleInputChange} placeholder="e.g., Senior Data Scientist" /></div>
                        <div className="form-group"><label htmlFor="bio">Bio</label><textarea id="bio" name="bio" value={profile.bio} onChange={handleInputChange} rows="5" placeholder="Tell us about yourself..."/></div>
                        <div className="edit-actions"><button type="submit" className="button-primary" disabled={isSaving}>{isSaving ? <><Loader size={16} className="animate-spin"/> Saving...</> : <><Save size={16}/> Save Profile</>}</button></div>
                    </form>
                </div>
            </div>
        </>
...
const UserProfilePage = ({ userId, onNavigate, currentUser }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (profileError) console.error("Profile fetch error:", profileError); else setProfile(profileData);
            ...
            if (projectsError) console.error("Projects fetch error:", projectsError); else setProjects(projectsData);
            setLoading(false);
        };
        fetchData();
    }, [userId]);
    ...
    return (
        <>
                ...
                <div className="profile-header">
                    ...
                    {isOwner && (
                        <div className="profile-actions">
                            <button onClick={() => setIsSettingsOpen(true)} className="action-button"><Settings size={16}/> Settings</button>
                            <button onClick={() => onNavigate('editProfile')} className="action-button"><Edit size={16}/> Edit Profile</button>
                        </div>
                    )}
                </div>
            ...
            {isOwner && <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />}
        </>
    );
};
...
const TimelinePage = ({ user, onNavigate }) => {
    ...
    return (
        <>
            <AppHeader onNavigate={onNavigate} user={user} />
            <div className="page-container">
                <div className="timeline-header"><h2>Timeline</h2><p>See the latest projects from the community.</p></div>
                {loading ? <LoadingSpinner /> : (
                    <div className="timeline-feed">
                        {projects.map(project => (
                            <div key={project.id} className="timeline-item">
                                <ProjectCard project={project} onClick={() => onNavigate('project', project.id)} />
                                <p className="timeline-author">Posted by <a href="#" onClick={(e) => {e.preventDefault(); onNavigate('profile', project.user_id)}}>{project.profiles?.username || 'a user'}</a></p>
                            </div>
                        ))}
                    </div>
                )}
...
const SearchPage = ({ user, onNavigate }) => {
    ...
    return (
        <>
            <AppHeader onNavigate={onNavigate} user={user} />
            <div className="page-container">
                <div className="search-header"><h2>Search</h2></div>
                <form onSubmit={handleSearch} className="search-form">
                    <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search for projects or users..." />
                    <button type="submit" disabled={loading}>{loading ? 'Searching...' : <Search size={18}/>}</button>
                </form>
                {loading ? <LoadingSpinner /> : (
                    <div className="search-results">
                        <div className="results-section">
                            <h3>Projects</h3>
                            <div className="projects-grid">
                                {results.projects.length > 0 ? results.projects.map(p => <ProjectCard key={p.id} project={p} onClick={() => onNavigate('project', p.id)} />) : <p>No projects found.</p>}
                            </div>
                        </div>
                        <div className="results-section">
                            <h3>Users</h3>
                            <div className="user-list">
                                {results.users.length > 0 ? results.users.map(u => (
                                    <div key={u.id} className="user-card" onClick={() => onNavigate('profile', u.id)}>
                                        <User size={24} />
...
const SettingsModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content">
                <div className="modal-header"><h3>Settings</h3><button type="button" onClick={onClose} className="close-button"><X size={20} /></button></div>
                <div className="modal-body">
                    <div className="setting-item">
                        <span>Accent Color</span>
                        <div className="color-picker">
                            {accentColors.map(color => (
                                <button key={color} style={{ backgroundColor: color }} className={`color-swatch ${activeColor === color ? 'active' : ''}`} onClick={() => changeAccentColor(color)} />
                            ))}
                        </div>
                    </div>
                </div>
...
export default function App() {
    ...
    const handleNavigate = (page, id = null) => { setView({ name: page, id }); };
